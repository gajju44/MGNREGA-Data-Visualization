		...
		<div className="bg-green-50 rounded-xl p-6">
			<div className="flex flex-wrap gap-2 items-center justify-between pb-4 text-gray-700 font-karla">
				...
				<div className="flex flex-wrap items-center gap-4">
					...
					<button className="px-3 py-1 rounded border border-green-300 text-green-800 disabled:opacity-50" disabled={totalPages ? currentPage>=totalPages : !hasMore} onClick={()=> setCurrentPage(p=> p+1)}>{t('table_next')}</button>
					</div>
					<span className="text-sm">{t('table_rows_per_page')}</span>
			...
			{loading ? (
				<div className="text-center text-lg font-semibold text-gray-600">{t('table_loading')}</div>
			) : data.length > 0 ? (
				<>
				...
				<div className="text-gray-600">{t('table_empty')}</div>
			)}
		</div>
		</>
	);
